...
const cartSlice = createSlice({
    ...
    reducers:{
        addPersonToCart: (state, action: PayloadAction<Person>) => {
            ...
            const itemInCart = state.itemsInCart.find((itemInCart: ItemInCart) => itemInCart.id === id)
            if(itemInCart) {
                itemInCart.count++
            } else {
        ...
        removePersonFromCart: (state, action: PayloadAction<Person>) =>{
            ...
            if(itemInCart) {
                if(itemInCart.count !== 1 ){
                    ...
                    state.itemsInCart = state.itemsInCart.filter((itemInCart: ItemInCart) => itemInCart.id !== id )
                }
            }
        },
...
export const {addPersonToCart, removePersonFromCart, removeCurPersFromCart, removeAllFromCart} = cartSlice.actions; 
