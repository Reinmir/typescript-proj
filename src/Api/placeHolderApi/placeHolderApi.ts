...
export class PlaceHolderApi {
   ...
   static async getToDos(): Promise<ToDoValue[]> {
    ...
    const { data } = await axios.get<ToDoValue[]>(`${PlaceHolderApi.baseUrl}/todos`);
    return data;
   ...
   static async exacToDo (id: number): Promise<ToDoValue>{
      ...
      const { data } = await axios.get<ToDoValue>(`${PlaceHolderApi.baseUrl}/todos/${id}`) 
      // console.log(data);
      return data; 
   ...
   static async getUser (userId: number): Promise<UserToDo>{
      const { data } = await axios.get<UserToDo>(`${PlaceHolderApi.baseUrl}/users/${userId}`)
      return data;
   }
